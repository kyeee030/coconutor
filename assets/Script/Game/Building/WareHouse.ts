...
export default class WareHouse extends Building {
    ...
    start() {
        ...
        this.headNode = null;
        this._targetNode = null; // 初始化目標節點為 null
    ...
    update(dt) {
        super.update(dt);
        if(this._targetNode !== null && this.headNode !== null) {
    ...
    onBeginContact(contact, selfCollider, otherCollider) {
        if (otherCollider.node.group === "Enemy") {
            console.log("WareHouse hit an enemy!");
        } else if (otherCollider.node.group === "Building") {
            console.log("WareHouse hit a building, no damage is applied.");
        }
    }
}
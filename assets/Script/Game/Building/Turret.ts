...
export default class Turret extends Building {
    ...
    start() {
        super.start(); 
        this.buildingType = "turret"; // 設置建築物類型
    ...
    // attack(): void {
    //     console.log("Turret is attacking!");
    //     this._animation.play("TurretAttack");
    //     super.attack(); // 呼叫父類別的攻擊方法
    // }

    // createBullet(): void {
    //     console.log(`Turret creates a bullet with damage:`);
    ...
    updateProperties(hp: number, damage: number, attackRange: number , level: number): void {
        ...
        console.log(`Turret updated: HP=${this.hp}, Damage=${this.damage}, Range=${this.attackRange}`);
    }

    // onBeginContact(contact, selfCollider, otherCollider) {
    //     if (otherCollider.node.group === "Enemy") {
    //         console.log("Turret hit an enemy!");
    //     } else if (otherCollider.node.group === "Building") {
    //         console.log("Turret hit a building, no damage is applied.");
    //     }
    // }
}